...
function timeAgo(
  dateString: string,
  t: (key: string, defaultVal?: Record<string, any>) => string,
): string {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p
            className={`
            text-sm leading-snug text-base-content
            ${n.isRead ? "font-normal opacity-70" : "font-semibold"}
          `}
          >
            {n.title}
          </p>
        ...
        <div className="flex items-center gap-2 mt-1.5">
          <span
            className={`badge badge-xs px-2.5 py-0.5 font-semibold ${TYPE_BADGE[n.type]}`}
          >
            {n.type.replace("_", " ")}
          </span>
...
export function NotificationCenter({ businessId }: { businessId: string }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchUnreadCount(businessId), 60_000);
  ...
  const handleOpen = () => {
    const next = !open;
    setOpen(next);
    if (next) fetchNotifications(businessId);
  };
        ...
        <>
          ...
          <div
            className="
                        ...
                        w-full h-full sm:w-[420px] sm:h-auto
                        sm:max-h-[540px]
            ...
            <div className="flex-1 overflow-y-auto bg-base-100">
              {loading ? (
                <div className="flex flex-col items-center justify-center gap-3 py-16 text-base-content/50">
                  <span className=" loading loading-spinner loading-sm animate-ping text-secondary" />
